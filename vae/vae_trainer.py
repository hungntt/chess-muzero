...
class VaeTrainer:
    def __init__(self, config, preprocesser):
        ...
        self.vae_name = self.config['forced_vae'] if self.config['forced_vae'] else self.config[
            'experiment_name'].replace('iterative_', '')
        ...
        self.checkpoint_filename = "checkpoints/" + self.vae_name + '_' + self.config["vae_trainer"][
    ...
    def load_data(self):  # To avoid reloading in constructor when not training
        self.train_dataset = RolloutObservationDataset(self.data_dir, self.preprocessor.normalize_frames_train,
                                                       buffer_size=self.train_buffer_size)
        self.test_dataset = RolloutObservationDataset(self.data_dir, self.preprocessor.normalize_frames_test,
                                                      buffer_size=self.test_buffer_size, is_train=False)
    ...
    def _reload_training_session(self):
        ...
        if exists(reload_file) and self.config['vae_trainer']['is_continue_model']:
            ...
            if exists(best_file):
                ...
                print(
                    f"Reloading vae at epoch {state['epoch']}, with best test error {best_test_loss} at epoch {best_state['epoch']}")
            else:
