    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                print(
                    f'Last test reward: {info["total_reward"]:.2f}. Final reward: {info["final_reward"]:.2f}. Training step: {info["training_step"]}/{self.config.training_steps}. Played games: {info["num_played_games"]}. Loss: {info["total_loss"]:.2f}',
                    end="\r",
                )
                counter += 1
                time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    if args_parser.game is not None:
        # Train directly with "python muzero.py cartpole"
        muzero = MuZero(args_parser.game)
        muzero.train()
    else:
